...
| `{{金_状态}}` | 金环状态 | 成 / 锈 / 断 |
| `{{金_表现}}` | 金环具体表现 | 10字以内摘要 |
| `{{金_边框色}}` | 边框颜色 | 成=#D4A017, 锈=#FF9800, 断=#F44336 |
| `{{金_线宽}}` | 线宽 | 成=3, 锈=2, 断=3 |
| `{{金_虚线}}` | 虚线样式 | 成=none, 锈=5,3, 断=none |
| `{{金_状态色}}` | 状态文字色 | 成=#4CAF50, 锈=#FF9800, 断=#F44336 |
...
## 4. 断链与修复序列表（Markdown 模板）

```markdown
| 五行环 | 当前状态 | 断链判定 | 具体表现 | 修复优先级 | 预计周期 |
|--------|---------|---------|---------|-----------|---------|
| 之金·下刀 | {{金_状态}} | {{金_是否断链}} | {{金_表现描述}} | {{金_优先级}} | {{金_周期}} |
| 行水·引水 | {{水_状态}} | {{水_是否断链}} | {{水_表现描述}} | {{水_优先级}} | {{水_周期}} |
| 有木·植树 | {{木_状态}} | {{木_是否断链}} | {{木_表现描述}} | {{木_优先级}} | {{木_周期}} |
| 效火·点火 | {{火_状态}} | {{火_是否断链}} | {{火_表现描述}} | {{火_优先级}} | {{火_周期}} |
| 归土·归藏 | {{土_状态}} | {{土_是否断链}} | {{土_表现描述}} | {{土_优先级}} | {{土_周期}} |

**主断链点**：{{主断链环}}
**修复顺序**：{{修复顺序}}（严禁乱序，只能顺行金→水→木→火→土）
...
![配图说明](assets/XX_配图名.png)（04_鞍点地形图 例外，使用 .svg 格式）
...
```
品牌名称 = 某咖啡品牌
金_状态 = 成    金_边框色 = #D4A017  金_线宽 = 3   金_虚线 = none    金_状态色 = #4CAF50
金_表现 = 定位清晰

水_状态 = 锈    水_边框色 = #FF9800  水_线宽 = 2   水_虚线 = 5,3    水_状态色 = #FF9800
水_表现 = 泵抽式引水

木_状态 = 断    木_边框色 = #F44336  木_线宽 = 3   木_虚线 = none    木_状态色 = #F44336
木_表现 = 全代工无壁垒

火_状态 = 锈    火_边框色 = #FF9800  火_线宽 = 2   火_虚线 = 5,3    火_状态色 = #FF9800
火_表现 = 爆发不沉淀

土_状态 = 断    土_边框色 = #F44336  土_线宽 = 3   土_虚线 = none    土_状态色 = #F44336
土_表现 = 无复购资产
